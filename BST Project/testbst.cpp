...
//testing erase
void test_lylas304() {
  cout << "Testing Bonus Extra Credit for Erase!" << endl;
  ...
  assert(b.erase("banana") == true);
  assert(b.erase("date") == false);

  assert(b.size() == 2);
  assert(b.contains("apple") == true);
  assert(b.contains("banana") == false);
  assert(b.contains("cherry") == true);
...
void test_lylas305() {
  ...
  cout << "Testing for count" << endl;
  BSTMap b;

  b["banana"] = 20;

  size_t countBanana = b.count("banana");
  assert(countBanana == 1);

  size_t countCherry = b.count("cherry");
  assert(countCherry == 0);
