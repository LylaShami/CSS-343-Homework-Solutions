...
private:
  ...
  void dfsRecursive(stack<string> &dfsStack, unordered_set<string> &visited,
                    void visit(const string &label));

  void dijkstraHelper(const string &startLabel, map<string, int> &vertexWeights,
                      map<string, string> &prevVertex) const;

  int mstPrimHelper(const string &startLabel, set<string> &visited,
...
public:
 ...
 bool connect(const string &sPoint, const string &ePoint, int weight = 0);

  // Remove edge from graph
  // @return true if edge successfully deleted
bool disconnect(const string &sPoint, const string &ePoint);
